...
async function sendPinEmail(email, pin) {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Pri Ruke <noreply@noviydim.sk>',
        to: [email],
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (!email || !customerId) {
          console.warn('checkout.session.completed missing email or customerId.');
          break;
        }

        await kvSet(`customer_email:${customerId}`, email);

        // Only issue a fresh PIN the first time this email pays — repeat
        // checkouts (e.g. reactivating a cancelled subscription) shouldn't
        // silently invalidate a PIN the person already knows.
        const existing = await kvGet(`access:${email}`);
      ...
      case 'customer.subscription.updated': {
        ...
        await updateSubscriptionByCustomerId(sub.customer, { status });
        break;
      }

      case 'customer.subscription.deleted': {
        const sub = event.data.object;
        await updateSubscriptionByCustomerId(sub.customer, { status: 'inactive' });
        break;
      }
      ...
      case 'invoice.payment_failed': {
        ...
        if (invoice.customer) {
          await updateSubscriptionByCustomerId(invoice.customer, { status: 'inactive' });
        }
        break;
      }

      default:
        // Ignore other event types — we don't need them.
        break;
    }
    ...
    // Still return 200 so Stripe doesn't hammer retries on a transient KV
    // blip — the individual writes above either succeeded or didn't, this
    // just avoids turning one failed field update into an infinite retry loop.
    return response.status(200).json({ received: true, warning: 'Processed with errors.' });
  }
}
